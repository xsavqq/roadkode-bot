...
main_menu = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="🛒 Новый заказ"), KeyboardButton(text="📨 Мои заявки")],
        [KeyboardButton(text="👤 Личный кабинет"), KeyboardButton(text="🔄 Обновить статусы")],
...
# ---------- Кнопки под отдельным товаром в корзине ----------
...
# ---------- Закрытый чат/канал ----------
closed_links_kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="💬 Закрытый чат", url=CLOSED_CHAT_URL)],
    [InlineKeyboardButton(text="📢 Закрытый канал", url=CLOSED_CHANNEL_URL)],
])

# ---------- Статусы заказа для менеджера ----------
STATUS_OPTIONS = ["В обработке", "Выкуплен", "На складе в Китае", "Отправлен", "Доставлен", "Отменён"]

def admin_status_kb(order_id: int) -> InlineKeyboardMarkup:
    rows = []
    row = []
    for i, status in enumerate(STATUS_OPTIONS, 1):
        row.append(InlineKeyboardButton(text=status, callback_data=f"set_status:{order_id}:{status}"))
        if i % 2 == 0:
            rows.append(row)
            row = []
    if row:
